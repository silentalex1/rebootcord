...
function loadDB() {
  try { if (fs.existsSync(DB_FILE)) { const d=JSON.parse(fs.readFileSync(DB_FILE,'utf8')); if(!d.changelogs) d.changelogs=[]; return d; } } catch(e) {}
  return { users: [], inviteCodes: {}, blacklisted: [], mcPorts: 25565, changelogs: [] };
}
...
app.post('/api/changelogs', (req, res) => {
  ...
  db.changelogs.unshift({ id: Date.now(), title: title.trim(), body: body.trim(), author: u, ts: Date.now(), likes: [] });
  saveDB();
  res.json({ success: true });
});
