...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    const forceOllama = AI_PROVIDER === 'ollama' || modelLower.includes('llava') || modelLower.includes('vision') || modelLower.includes('ollama') || modelLower === 'rebootcord-vision';
    const useOllama = forceOllama || AI_PROVIDER === 'ollama' || (!GEMINI_API_KEY && AI_PROVIDER !== 'gemini') || hasImages;
    ...
    if (useOllama) {
      try {
        ...
        const msg = 'PrysmisAI is thinking.. Make sure its running on port 1000';
      ...
      return res.json({
        ...
        choices: [{
          index: 0,
          message: { role: 'assistant', content: 'PrysmisAI is active via the local model. Make sure you ran ollama pull llava:7b and npm run ollama:create-vision (or the script), then set OLLAMA_VISION_MODEL=prysmis-ai and restart.' },
          finish_reason: 'stop'
        }],
...
app.post('/v1/messages', async (req, res) => {
  try {
    ...
    if (useOllama) {
      ...
      return res.status(500).json({ error: 'PrysmisAI active. Run the pull and create commands, set OLLAMA_VISION_MODEL=prysmis-ai.' });
    } else {
      result = await callGemini(requestedModel, messages, body);
    }
...
server.listen(PORT, () => { console.log('Reboot Cord running on port ' + PORT); console.log('PrysmisAI model: ' + OLLAMA_VISION_MODEL + ' (port 1000 default, also works on rebootcord.world)'); });
