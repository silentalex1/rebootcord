...
const { sendWelcomeEmail, isValidEmail, sendMail, renderTemplate } = require('./email');
...
if (!process.env.SESSION_SECRET) {
  console.warn('SESSION_SECRET is not set. Set it in your environment before deploying to production.');
}
...
function loadDB() {
  try { if (fs.existsSync(DB_FILE)) { const d=JSON.parse(fs.readFileSync(DB_FILE,'utf8')); if(!d.changelogs) d.changelogs=[]; if(!d.apiKeys) d.apiKeys=[]; if(!d.feedbacks) d.feedbacks=[]; if(!d.feedbackChats) d.feedbackChats={}; if(!d.inboxMessages) d.inboxMessages=[]; if(!d.shareInvites) d.shareInvites=[]; if(!d.mcClientServers) d.mcClientServers=[]; if(!d.emailConfigs) d.emailConfigs={}; return d; } } catch(e) {}
  return { users: [], inviteCodes: {}, blacklisted: [], mcPorts: 25565, changelogs: [], apiKeys: [], feedbacks: [], feedbackChats: {}, inboxMessages: [], shareInvites: [], mcClientServers: [], emailConfigs: {} };
}
...
app.get('/api/email-system/config', (req, res) => {
  ...
  res.json({ success: true, config: cfg || { fromName: '', subject: '', message: '', enabled: false } });
});
...
app.post('/api/email-system/config', (req, res) => {
  ...
  db.emailConfigs[u] = { fromName, subject, message, enabled, updated: new Date().toISOString() };
  saveDB();
  res.json({ success: true, config: db.emailConfigs[u] });
});
...
app.post('/api/email-system/test', emailSystemLimiter, async (req, res) => {
  ...
  if (!cfg || !cfg.enabled) return res.json({ success: false, message: 'Email system is not enabled yet' });
  if (!isValidEmail(req.body.to)) return res.json({ success: false, message: 'Invalid recipient email' });
  const vars = { name: sanitizeEmailField(req.body.name, 60) || 'there', email: req.body.to };
