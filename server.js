...
function spawnBot(username, name) {
    ...
    const nodeModules = path.join(botDir, 'node_modules');
    ...
    const env = {
        ...
        PYTHONPATH: [pkgDir, existingPyPath].filter(Boolean).join(':')
    };
...
app.post('/api/servers/:username/:name/files/upload', (req, res) => {
    const { username, name } = req.params;
    const project = projects.find(p => p.owner === username && p.slug === name);
    if (!project) return res.json({ success: false });
    const botDir = path.join(BOTS_DIR, username, name);
    ...
    bb.on('file', (field, stream, info) => {
        // field name carries relative path for folder uploads
        const relPath = (info.filename || field).replace(/[\\]+/g, '/').replace(/^[/]+/, '');
        const fullPath = safePath(botDir, relPath);
        if (!fullPath) { stream.resume(); return; }
        fs.mkdirSync(path.dirname(fullPath), { recursive: true });
        const w = new Promise((res2, rej2) => {
            const ws = fs.createWriteStream(fullPath);
            stream.pipe(ws);
            ws.on('finish', () => { uploaded.push(relPath); res2(); });
            ws.on('error', rej2);
        });
        writes.push(w);
    });
...
app.post('/api/servers/:username/:name/install', (req, res) => {
    ...
    proc.on('close', code => {
        pushLog(key, code === 0 ? '[PKGDONE] ' + pkgName + ' package has been installed into the bot.' : '[SYS] Install failed (code ' + code + ')');
    });
    res.json({ success: true });
});
