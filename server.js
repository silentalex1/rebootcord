...
function loadDB() {
  try { if (fs.existsSync(DB_FILE)) { const d=JSON.parse(fs.readFileSync(DB_FILE,'utf8')); if(!d.changelogs) d.changelogs=[]; if(!d.apiKeys) d.apiKeys=[]; return d; } } catch(e) {}
  return { users: [], inviteCodes: {}, blacklisted: [], mcPorts: 25565, changelogs: [], apiKeys: [] };
}
        ...
        if (p) {
          ...
          if (p.lang === 'Python') {
            ...
            if (pkgs.length > 0) {
              ...
              pkgs.forEach(pk => {
                let p = pk;
                const map = { "pil": "Pillow", "pillow": "Pillow", "dotenv": "python-dotenv", "bs4": "beautifulsoup4", "cv2": "opencv-python", "yaml": "PyYAML", "sqlite": "aiosqlite" };
                if (map[p]) p = map[p];
                set.add(p);
              });
              // sanitize entire set for bad package names like "pil"
          ...
          if (p.lang === 'Python') {
            ...
            const installNext = (i) => {
              ...
              cp.exec(`pip install ${pk} --target ./modules`, { cwd: pDir, shell: true }, (err, stdout, stderr) => {
                if (stdout) broadcastLog(user, p.id, stdout, 'info');
                if (stderr) broadcastLog(user, p.id, stderr, 'warn');
                if (err) {
...
app.post('/api/projects/:id/start', async (req, res) => {
  ...
  if (p.type === 'minecraft') {
    ...
    // ensure .env works if user created .env file or used token box
    try {
    ...
    proc.on('close', () => { 
      ...
      if (missingPkgs.size > 0) {
        missingPkgs.forEach(pkg => broadcastLog(u, p.id, 'Missing package: ' + pkg, 'sys'));
      }
...
app.post('/api/v1/deploy', (req, res) => {
  const u = getUser(req);
  if (!u) return res.json({ success: false, message: 'Login required' });
  res.json({ success: true, message: 'Deploy request received. Hosting magic started.' });
});
