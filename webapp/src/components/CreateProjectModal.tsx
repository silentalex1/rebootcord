...
export function CreateProjectModal({
  ...
  const handleSubmit = async () => {
    ...
    if (!trimmed || type === "minecraft") {
      if (!trimmed) setError("Give your project a name.");
      return;
    }
    setSubmitting(true);
    setError("");
  ...
  return (
    <Modal title="New project" onClose={onClose}>
      ...
      <div className="mb-4">
        <label htmlFor="project-name-input" className="mb-1.5 block text-[10px] font-bold uppercase tracking-wider text-text-mute">
          Project name
        </label>
        <input
          value={name}
      ...
      {type === "discord" ? (
        ...
        <div className="mb-2 flex flex-col items-center gap-2 rounded-xl border border-dashed border-border-bright bg-surface-2 py-8 text-center">
          <Server size={20} className="text-text-mute" />
          <p className="text-[13px] font-bold text-text-dim">Coming soon</p>
          <p className="max-w-[220px] text-[11.5px] text-text-mute">
            Minecraft server hosting isn't available yet.
          </p>
        </div>
      )}
      ...
      <button
        ...
        disabled={submitting || type === "minecraft"}
      ...
      >
        {type === "minecraft" ? "Coming soon" : submitting ? "Creating..." : "Create project"}
      </button>
    </Modal>
  );
}
