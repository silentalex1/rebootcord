...
export function ProjectPage() {
  ...
  const [installingOne, setInstallingOne] = useState(false);
  ...
  useEffect(() => {
    ...
    return rcSocket.subscribe((data) => {
      ...
      } else if (data.event === "installDone") {
        setInstallingOne(false);
        push(data.success ? `Installed ${data.pkg}` : `Failed to install ${data.pkg}`, data.success ? "success" : "error");
  ...
  const installOne = () => {
    const pkg = depInput.trim();
    if (!pkg || !projectId) return;
    setInstallingOne(true);
    setTab("console");
    rcSocket.send({ event: "install", projectId, pkg });
    setDepInput("");
  };
  ...
  return (
    <div className="flex min-h-screen flex-col">
          ...
          <div className="border-t border-border p-3">
            ...
            <div className="flex items-center gap-1.5">
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && installOne()}
                placeholder="e.g. requests"
                className="min-w-0 flex-1 rounded-lg border border-border bg-surface-2 px-2.5 py-2 text-[12px] outline-none focus:border-red"
              />
            </div>
            <button
              onClick={installOne}
              disabled={installingOne || !depInput.trim()}
              className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg bg-red px-3 py-2 text-[12px] font-bold text-white transition hover:bg-red-dark disabled:opacity-50"
            >
              {installingOne ? <Loader2 size={13} className="animate-spin" /> : <Wrench size={13} />}
              Install dependency
            </button>
          </div>
        </aside>
      ...
      {showAi && project.type === "discord" && (
        <AiPanel
          title="Reboot Code AI"
