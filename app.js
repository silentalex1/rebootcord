  ...
  const page = el("div", { className: "projects-page" },
    el("div", { className: "projects-header" },
      ...
      el("div", { style: { display: "flex", gap: "10px", alignItems: "center" } },
        ...
        el("button", { className: "btn-changelogs", onClick: function(){ history.pushState(null, "", "/dashboard/changelogs"); state.page = "changelogs"; fetchChangelogs(); scheduleRender(); } }, "Changelogs"),
...
function renderChangelogsPage() {
  ...
  if (state.isAdmin) {
    header.appendChild(el("div", { style: { display: "flex", gap: "10px", alignItems: "center" } },
      ...
      el("button", { className: "btn-cancel", onClick: () => { history.pushState(null, "", "/dashboard"); state.page = "projects"; scheduleRender(); } }, "Go Back")
    ));
  } else {
    header.appendChild(el("div", { style: { display: "flex", gap: "10px", alignItems: "center" } },
      el("button", { className: "btn-cancel", onClick: () => { history.pushState(null, "", "/dashboard"); state.page = "projects"; scheduleRender(); } }, "Go Back")
  ...
  if (!state.changelogs || state.changelogs.length === 0) {
    ...
    state.changelogs.forEach(function(ch) {
      ...
      const card = el("div", { className: "changelog-card" },
        el("div", { className: "ch-title" }, ch.title),
        el("div", { className: "ch-body" }),
        el("div", { className: "ch-meta" }, "by " + ch.author + " • " + new Date(ch.ts).toLocaleString())
      );
      ...
      const heartWrap = el("div", { className: "heart-wrap", style: { marginTop: "10px", display: "inline-flex", alignItems: "center", gap: "6px" } },
        el("button", { className: "heart-btn", onClick: () => toggleLike(ch.id) }, "♥"),
...
function renderOurApi() {
  ...
  const header = el("div", { className: "projects-header" },
    el("div", { className: "projects-title" },
      el("h1", {}, "Reboot API"),
      el("p", {}, "For developers. Extend the platform.")
    )
  );
  wrap.appendChild(header);
  const container = el("div", { style: "display:flex; gap:20px" });
  const sidebar = el("div", { className: "api-sidebar", style: "width:200px; flex-shrink:0; border-right:2px solid #333; padding-right:12px; position:relative" });
  ...
  navItems.forEach((item, idx) => {
    ...
    if (idx < navItems.length - 1) {
      const line = el("div", { style: "height:1px; background:#333; margin:2px 0 2px 10px; width:2px" });
  ...
  const sections = [
    { id: "overview", title: "Home Overview", content: () => {
      ...
      const btn = el("button", { className: "btn-new", onClick: createApiKey }, "Create API Key");
      d.appendChild(btn);
      if (state.recentKey) {
        const box = el("div", { style: "margin-top:12px; padding:12px; background:#111; border:1px solid #2d2d2d; border-radius:8px" });
        box.appendChild(el("div", { style: "color:#e63946; font-weight:700" }, "Copy this key now (shown once):"));
        const keyEl = el("div", { style: "font-family:monospace; word-break:break-all; margin:8px 0" }, state.recentKey);
        box.appendChild(keyEl);
        const copyB = el("button", { className: "btn-cancel", onClick: () => { navigator.clipboard.writeText(state.recentKey); } }, "Copy");
        const hideB = el("button", { className: "btn-cancel", onClick: () => { state.recentKey = null; scheduleRender(); } }, "Hide Key");
        box.appendChild(copyB);
        box.appendChild(hideB);
        d.appendChild(box);
      }
      d.appendChild(el("div", { style: "margin-top:16px; font-weight:700" }, "Your API Keys"));
      if (!state.apiKeys || state.apiKeys.length === 0) {
        d.appendChild(el("div", { style: "color:#888; font-size:12px" }, "No keys yet."));
      } else {
        state.apiKeys.forEach(k => {
          const row = el("div", { style: "margin:6px 0; padding:6px; background:#111; border-radius:6px; display:flex; justify-content:space-between; align-items:center; font-size:12px" });
          row.appendChild(el("span", { style: "font-family:monospace" }, "rc_*****"));
          const copyM = el("button", { className: "btn-cancel", style: "font-size:10px; padding:2px 6px" , onClick: () => { navigator.clipboard.writeText("rc_live_****************"); } }, "Copy masked");
          row.appendChild(copyM);
          d.appendChild(row);
        });
      }
      return d;
    } },
    { id: "hosting-magic", title: "Hosting Magic", content: () => el("div", {}, el("p", {}, "Use the deploy endpoint to let the platform host your projects automatically.")) },
    { id: "api-endpoints", title: "API Endpoints", content: () => {
      const d = el("div");
      d.appendChild(el("div", { style: "margin:8px 0; padding:10px; background:#111; border-radius:8px" },
        el("div", { style: "font-weight:700" }, "POST /api/v1/deploy"),
        el("div", { style: "font-size:12px; color:#aaa; margin-top:4px" }, "This api will hook your site project, so that your site will send its calls to the api provided, so you can just sit back and watch the host magic happening in real time.")
      ));
      d.appendChild(el("div", { style: "margin:8px 0; padding:10px; background:#111; border-radius:8px" },
        el("div", { style: "font-weight:700" }, "POST /api/v1/apikeys"),
        el("div", { style: "font-size:12px; color:#aaa; margin-top:4px" }, "Create a new API key for your apps.")
      ));
      return d;
    } },
    { id: "sdks-autostyling", title: "SDKs - AutoStyling", content: () => {
      const d = el("div");
      d.appendChild(el("p", {}, "Include this in your site:"));
      const ta = el("textarea", { className: "about-editor", style: "width:100%; height:60px; font-family:monospace; font-size:12px" }, '<script src="https://rebootcord.onrender.com/sdk/rebootui.js"></script>');
      ta.readOnly = true;
      d.appendChild(ta);
      d.appendChild(el("p", { style: "margin-top:8px" }, "Then init:"));
      const initTa = el("textarea", { className: "about-editor", style: "width:100%; height:50px; font-family:monospace; font-size:12px" }, 'RebootUI.init({ apiKey: "YOUR_API_KEY" });');
      initTa.readOnly = true;
      d.appendChild(initTa);
      d.appendChild(el("p", {}, "Example:"));
      const ex = el("div", { style: "padding:8px; background:#111; border-radius:6px; font-family:monospace; font-size:12px" }, '<div class="user-card">Welcome</div>\n\nBecomes:\n<div class="rb-card rb-glass rb-hover">Welcome</div>');
      d.appendChild(ex);
      d.appendChild(el("p", { style: "margin-top:8px" }, "Or:"));
      const codeEx = el("textarea", { className: "about-editor", style: "width:100%; height:40px; font-family:monospace; font-size:12px" }, 'RebootUI.createCard({ title: "Bot", description: "Online" });\nRebootUI.page({ theme: "cyber", glass: true });');
      codeEx.readOnly = true;
      d.appendChild(codeEx);
    ...
    { id: "javascript", title: "Javascript", content: () => el("div", {}, el("p", {}, "Use the SDK script above for full auto UI polish in JS.")) }
  ];
  ...
  setTimeout(() => {
    const onScroll = () => {
      ...
      if (cur) history.replaceState(null, "", "/ourapi/#" + cur);
    };
...
function renderChangelogModal() {
  ...
  const modal = el("div", { className: "modal changelog-modal" },
    el("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" } },
      ...
      el("button", { className: "btn-cancel", style: { padding: "4px 10px", fontSize: "18px" }, onClick: () => { state.showChangelogModal = false; scheduleRender(); } }, "×")
    ),
...
document.addEventListener("keydown", function(e) {
  if ((e.ctrlKey || e.metaKey) && (e.key === "s" || e.key === "S")) {
    if (state.page === "bot-dashboard" && state.currentProject && state.editorFile) {
      ...
      if (toolbar) {
        const saveBtn = toolbar.querySelector(".btn-save-file");
        if (saveBtn) {
