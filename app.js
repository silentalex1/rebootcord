    ...
    try {
      ...
      if (data.event === 'log') {
        ...
        if (data.msg && (data.msg.indexOf("Process exited") !== -1 || data.msg.indexOf("forcefully killed") !== -1 || data.msg.indexOf("stopped manually") !== -1)) {
          ...
          scheduleRender();
        } else if (state.currentProject && String(state.currentProject.id) === String(data.projectId)) {
          scheduleRender();
        }
      }
    } catch(err) {}
...
function getDefaultCode(p) {
  if (p.lang === "Python") return "import os\n\n# Token is automatically provided via env var (from the token box) or .env\nTOKEN = os.getenv('BOT_TOKEN') or os.getenv('TOKEN')\n\nprint('Bot starting...')\n\n# Example:\n# import discord\n# from discord.ext import commands\n# intents = discord.Intents.default()\n# bot = commands.Bot(command_prefix='!', intents=intents)\n# @bot.event\n# async def on_ready():\n#     print(f'Logged in as {bot.user}')\n# bot.run(TOKEN)\n";
...
function installPkg() {
  ...
  if (state.currentProject.lang === "Python") {
    const pkgMap = { "discord": "discord.py", "pil": "Pillow", "pillow": "Pillow", "dotenv": "python-dotenv", "bs4": "beautifulsoup4", "cv2": "opencv-python" };
    if (pkgMap[v.toLowerCase()]) v = pkgMap[v.toLowerCase()];
  }
...
function removeInstalledPkg(pkg) {
  if (!state.currentProject) return;
  const p = state.currentProject;
  if (p.lang === "Python") {
    const reqFile = "requirements.txt";
    let content = (p.files && p.files[reqFile]) || "";
    const lines = content.split(/\r?\n/).filter(l => l.trim() !== pkg && l.trim() !== "");
    const newContent = lines.join("\n") + (lines.length ? "\n" : "");
    p.files = p.files || {};
    p.files[reqFile] = newContent;
  } else if (p.lang === "JavaScript" || p.lang === "TypeScript") {
    const pkgFile = "package.json";
    let content = (p.files && p.files[pkgFile]) || "{}";
    try {
      const parsed = JSON.parse(content);
      if (parsed.dependencies && parsed.dependencies[pkg]) {
        delete parsed.dependencies[pkg];
        p.files = p.files || {};
        p.files[pkgFile] = JSON.stringify(parsed, null, 2);
      }
    } catch (e) {}
  }
  const proj = state.projects.find(x => x.id === p.id);
  if (proj) proj.files = p.files;
  saveProjects();
  scheduleRender();
}
  ...
  frag.appendChild(el("div", { className: "dashboard discord-dash" },
    el("div", { className: "sidebar discord-sidebar" },
      ...
      el("div", { className: "sidebar-section" },
        ...
        installAllSection,
        (() => {
          const list = el("div", { style: { marginTop: "6px", fontSize: "10px" } });
          let pkgs = [];
          if (p.lang === "Python") {
            const req = (p.files && p.files["requirements.txt"]) || "";
            pkgs = req.split(/\r?\n/).map(l => l.trim()).filter(l => l && !l.startsWith("#"));
          } else if (p.lang === "JavaScript" || p.lang === "TypeScript") {
            try {
              const pkgJson = (p.files && p.files["package.json"]) || "{}";
              const parsed = JSON.parse(pkgJson);
              if (parsed.dependencies) {
                pkgs = Object.keys(parsed.dependencies);
              }
            } catch (e) {}
          }
          if (pkgs.length === 0) return null;
          pkgs.forEach(pk => {
            const row = el("div", { style: { display: "flex", alignItems: "center", gap: "4px", background: "#1a1a1a", padding: "2px 4px", borderRadius: "3px", margin: "2px 0" } },
              el("span", { style: { flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, pk),
              el("button", { className: "btn-clear", style: { padding: "1px 3px", fontSize: "9px", background: "#3a1a1a", color: "#ff6b6b", border: "1px solid #5c2a2a", borderRadius: "3px" }, onClick: () => { if (confirm("Remove " + pk + " from dependencies?")) removeInstalledPkg(pk); } }, svgIcon("trash"))
            );
...
function renderOurApi() {
  ...
  wrap.appendChild(header);
  const container = el("div", { className: "api-container" });
  const sidebar = el("div", { className: "api-sidebar" });
  ...
  const navItems = [
    ...
    { id: "ai-proxy", label: "AI Proxy (Gemini)" },
    { id: "ai-integration", label: "AI intergration" },
    { id: "sdks-autostyling", label: "SDKs - AutoStyling" },
    { id: "javascript", label: "Javascript" }
  ];
  navItems.forEach((item, idx) => {
    const link = el("div", { style: "padding:8px 12px; cursor:pointer; font-size:13px", "data-id": item.id, onClick: () => {
      document.querySelectorAll('.api-sidebar > div').forEach(d => d.classList.remove('active'));
  ...
  // initial active on overview
  const initActive = sidebar.querySelector('div[data-id="overview"]');
  ...
  const sections = [
    { id: "overview", title: "Home Overview", content: () => {
      ...
      d.appendChild(el("p", { style: { fontSize: "12px", color: "#aaa", marginTop: "6px" } }, "Keys start with rc_live_ but lists and hidden views show rc_****** for security. The copy key button always provides the full key for the latest created entry even when masked. Full key is only available in this session after creation."));
      ...
      if (state.recentKey) {
        ...
        const showB = el("button", { className: "btn-new", style: "font-size:10px; padding:3px 8px; background:#e63946; color:#fff; border:none; border-radius:4px;" }, "show api key");
        showB.onclick = () => { state.recentKeyHidden = false; scheduleRender(); };
        const copyB = el("button", { className: "btn-new", style: "font-size:10px; padding:3px 8px; background:#e63946; color:#fff; border:none; border-radius:4px;" }, "copy key");
        copyB.onclick = () => { navigator.clipboard.writeText(state.recentKey); const o = copyB.style.color; copyB.style.color = "var(--green)"; setTimeout(()=>{copyB.style.color=o;},700); };
      ...
      if (!state.apiKeys || state.apiKeys.length === 0) {
        ...
        state.apiKeys.forEach(k => {
          ...
          const showB = el("button", { className: "btn-new", style: "font-size:10px; padding:3px 8px; background:#e63946; color:#fff; border:none; border-radius:4px;" }, "show api key");
          showB.onclick = () => { alert("Full key only shown once at creation for security."); };
          const isRecent = state.lastCreatedKey && k.id === state.lastCreatedKey.id;
          const copyB = el("button", { className: "btn-new", style: "font-size:10px; padding:3px 8px; background:#e63946; color:#fff; border:none; border-radius:4px;" }, "copy key");
          copyB.onclick = () => { const toCopy = (isRecent && state.lastCreatedKey) ? state.lastCreatedKey.key : (k.masked || "rc_******"); navigator.clipboard.writeText(toCopy); const o = copyB.style.color; copyB.style.color = "var(--green)"; setTimeout(()=>{copyB.style.color=o;},700); };
          acts.appendChild(showB);
          acts.appendChild(copyB);
    ...
    { id: "api-endpoints", title: "API Endpoints", content: () => {
      ...
      d.appendChild(el("div", { style: "margin:8px 0; padding:14px; background:#0a0a0a; border:1px solid #222; border-radius:10px" },
        ...
        el("div", { style: "font-size:12px; color:#aaa; margin-top:6px" }, "This api will hook your site project, so that your site will send its calls to the api provided, so you can just sit back and watch the host magic happening in real time. Use your rc_live_ key in the request for auth.")
      ));
      d.appendChild(el("div", { style: "margin:8px 0; padding:14px; background:#0a0a0a; border:1px solid #222; border-radius:10px" },
        el("div", { style: "font-weight:700; color:#fff; font-size:14px" }, "POST /api/v1/apikeys"),
        el("div", { style: "font-size:12px; color:#aaa; margin-top:6px" }, "Create a new API key for your apps. Full key returned once only; use masked rc_****** in lists. Copy button gives full for recent even when hidden.")
      ));
      return d;
    } },
    { id: "sdks-autostyling", title: "SDKs - AutoStyling", content: () => {
      ...
      d.appendChild(el("div", { style: "margin-bottom:12px; padding:12px; background:#0a0a0a; border-radius:8px; border:1px solid #222" },
        ...
        (() => { const ta = el("textarea", { className: "api-code", style: "width:100%; height:60px; color:#f4f4f4; background:#0f0f0f" }, '<script src="https://rebootcord.world/sdk/rebootui.js"></script>'); ta.readOnly = true; ta.onclick = () => { navigator.clipboard.writeText(ta.value); const orig = ta.style.color; ta.style.color = "var(--green)"; setTimeout(() => { ta.style.color = orig; }, 800); }; return ta; })()
      ));
      d.appendChild(el("div", { style: "margin-bottom:12px; padding:12px; background:#0a0a0a; border-radius:8px; border:1px solid #222" },
        el("div", { style: "font-weight:600; margin-bottom:6px; color:#fff" }, "Then init:"),
        (() => { const initTa = el("textarea", { className: "api-code", style: "width:100%; height:50px; color:#f4f4f4; background:#0f0f0f" }, 'RebootUI.init({ apiKey: "YOUR_API_KEY", feedback:true });'); initTa.readOnly = true; initTa.onclick = () => { navigator.clipboard.writeText(initTa.value); const orig = initTa.style.color; initTa.style.color = "var(--green)"; setTimeout(() => { initTa.style.color = orig; }, 800); }; return initTa; })()
      ));
      d.appendChild(el("div", { style: "margin-bottom:12px; padding:12px; background:#0a0a0a; border-radius:8px; border:1px solid #222" },
      ...
      d.appendChild(el("p", { style: { fontSize: "12px", color: "#888", marginTop: "8px" } }, "Include the script tag, call RebootUI.init({apiKey: 'your_key', feedback:true}), then use classes like rb-card rb-glass or helpers. Feedback system included via submitFeedback/showFeedback. Only affects marked elements."));
      return d;
    } },
    { id: "ai-proxy", title: "AI Proxy (Gemini) - Add AI to Your Apps", content: () => {
      const d = el("div");
      d.appendChild(el("p", {}, "Use our custom Gemini proxy to easily add powerful AI (Gemini 2.5 Pro etc.) to your own websites, bots, or dashboards. No API key needed from you - the platform handles Gemini calls."));
      d.appendChild(el("p", { style: { fontWeight: "700" } }, "Base URL: https://rebootcord.onrender.com/v1"));
      d.appendChild(el("div", { style: { margin: "12px 0" } }, el("strong", {}, "Chat Completions (OpenAI compatible):"), el("div", { className: "api-code", style: "margin-top:4px" }, "POST https://rebootcord.onrender.com/v1/chat/completions")));
      d.appendChild(el("p", {}, "Example payload (use \"role\", \"content\", and \"AI model\": \"gemini 2.5 pro\" ):"));
      (() => { const ex = el("textarea", { className: "api-code", style: "width:100%; height:110px; color:#f4f4f4; background:#0f0f0f" }, `{
  "model": "gemini 2.5 pro",
  "messages": [
    {
      "role": "user",
      "content": "Hello, tell me about Reboot Cord"
    }
  ],
  "AI model": "gemini 2.5 pro"
}`); ex.readOnly = true; d.appendChild(ex); })();
      d.appendChild(el("div", { style: { margin: "12px 0" } }, el("strong", {}, "Messages Endpoint:"), el("div", { className: "api-code", style: "margin-top:4px" }, "POST https://rebootcord.onrender.com/v1/messages")));
      d.appendChild(el("p", { style: { fontSize: "12px", color: "#aaa" } }, "Custom robust translator (systemInstruction support, safety block handling, real usageMetadata tokens, rate limit 20/min per IP, \"AI model\" field fallback). Compatible with diwness-style examples."));
      d.appendChild(el("p", {}, "Supported: gemini-2.5-pro (\"gemini 2.5 pro\"), gemini-2.0-flash + Claude model ID auto-mapping."));
      return d;
    } },
    { id: "ai-integration", title: "AI intergration", content: () => {
      const d = el("div");
      d.appendChild(el("p", {}, "Integrate Gemini 2.5 Pro using our site proxy endpoint. No Gemini key needed on client side - our endpoint handles the model call securely and robustly."));
      d.appendChild(el("p", { style: { fontWeight: "700" } }, "Base endpoint: https://rebootcord.onrender.com/v1"));
      d.appendChild(el("p", {}, "POST to /v1/chat/completions (OpenAI shape) or /v1/messages. Always include role + content in messages. Use model or \"AI model\" set to \"gemini 2.5 pro\"."));
      (() => { const ex = el("textarea", { className: "api-code", style: "width:100%; height:96px; color:#f4f4f4; background:#0f0f0f" }, `{
  "model": "gemini 2.5 pro",
  "messages": [ { "role": "user", "content": "hello from my app" } ],
  "AI model": "gemini 2.5 pro",
  "temperature": 0.7
}`); ex.readOnly = true; d.appendChild(ex); })();
      d.appendChild(el("p", {}, "Example using fetch in browser or node:"));
      (() => { const ex2 = el("textarea", { className: "api-code", style: "width:100%; height:82px; color:#f4f4f4; background:#0f0f0f" }, `fetch("https://rebootcord.onrender.com/v1/chat/completions", {
  method:"POST", headers:{"Content-Type":"application/json"},
  body: JSON.stringify({ model:"gemini 2.5 pro", messages:[{role:"user",content:"hi"}], "AI model":"gemini 2.5 pro" })
}).then(r => r.json()).then(console.log);`); ex2.readOnly = true; d.appendChild(ex2); })();
      d.appendChild(el("p", { style: { fontSize: "12px", color: "#888" } }, "Also supports /v1/models to list available, system/developer roles, top_p, stop_sequences, retries + keepalive for stable connections, safety handling and usage stats returned."));
      d.appendChild(el("p", {}, "Use in any dashboard, external site, or your RebootCord projects for AI features powered by Gemini 2.5 Pro."));
      return d;
    } },
    { id: "javascript", title: "Javascript", content: () => el("div", { style: "padding:14px; background:#0a0a0a; border:1px solid #222; border-radius:10px" }, el("p", { style: "margin:0; color:#ccc; font-size:13px" }, "Use the SDK script above for full auto UI polish in JS. Call init then add rb- classes or use createCard/page for clean professional dashboards, cards, buttons without weird side effects on your existing layout."), el("div", { style: "margin-top:8px; padding:6px 8px; background:#111; font-family:monospace; font-size:10px; color:#0f0; border:1px solid #222; white-space:pre" }, "[RebootUI] Initialized with API key\n[RebootUI] Auto styles applied. Dashboards and UI enhanced.") ) }
  ];
  ...
  if (!state.sdkInjected) {
    ...
    setTimeout(() => {
      ...
      if (!existing) {
        ...
        s.onload = () => { if (window.RebootUI && !window.RebootUI._inited) { try { const root = document.querySelector(".api-container") || document.body; RebootUI.init({apiKey:"demo", root: root}); window.RebootUI._inited = true; const over = document.createElement("style"); over.textContent = ".rb-btn:hover{transform:none!important;filter:none!important} .rb-hover:hover{transform:none!important}"; document.head.appendChild(over); } catch(e){} } };
        document.head.appendChild(s);
      } else if (window.RebootUI && !window.RebootUI._inited) {
        try { const rt = document.querySelector(".api-container") || document.body; RebootUI.init({apiKey:"demo", root: rt}); window.RebootUI._inited = true; } catch(e){}
      }
    }, 80);
  }
...
function formatChangelogBody(body) {
  ...
  lines.forEach(function(line) {
    ...
    if (t.startsWith("#")) {
      ...
      html += "<div style=\"font-size:18px;font-weight:bold;margin:6px 0;\">" + escapeHtml(t.slice(1).trim()) + "</div>";
    } else if (t.startsWith(">") ) {
      if (inList) { html += "</ul>"; inList = false; }
      html += "<div style=\"border-left:3px solid #666;padding-left:8px;margin:6px 0;color:#aaa;\">" + escapeHtml(t.slice(1).trim()) + "</div>";
...
function renderChangelogModal() {
  ...
  const modal = el("div", { className: "modal changelog-modal", style: { display: "flex", gap: "16px", maxWidth: "720px" } },
    el("div", { style: { flex: "1" } },
      el("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" } },
        el("h2", { style: { margin: 0, fontSize: "16px" } }, "-- Post a changelog"),
        el("button", { className: "btn-cancel", style: { padding: "0px 4px", fontSize: "18px", lineHeight: "1", minWidth: "auto" }, onClick: () => { state.showChangelogModal = false; scheduleRender(); } }, "×")
      ),
    ...
    el("div", { style: { width: "220px", background: "#1a1a1a", borderRadius: "8px", padding: "10px", fontSize: "11px", color: "#aaa", border: "1px solid #333" } },
      el("div", { style: { fontWeight: "700", color: "#fff", marginBottom: "6px", fontSize: "12px" } }, "acceptable formats:"),
      el("div", { style: { marginBottom: "2px" } }, "if you put \"+\" that will turn into a bullet point upon posting."),
      el("div", { style: { fontFamily: "monospace", background: "#222", padding: "1px 3px", margin: "2px 0 6px 0", fontSize: "10px" } }, "+ text  →  • text"),
      el("div", { style: { marginBottom: "2px" } }, "if you put \">\" that will make that text have a gray line."),
      el("div", { style: { fontFamily: "monospace", background: "#222", padding: "1px 3px", margin: "2px 0 6px 0", fontSize: "10px", borderLeft: "3px solid #666", paddingLeft: "3px", color: "#888" } }, "> note  (gray)"),
      el("div", { style: { marginBottom: "2px" } }, "if you put \"#\" that will make the font of that text afterwords big. (example)"),
      el("div", { style: { fontFamily: "monospace", background: "#222", padding: "1px 3px", margin: "2px 0 6px 0", fontSize: "10px" } }, "# Big  →"),
      el("div", { style: { fontSize: "15px", fontWeight: "bold", margin: "0 0 6px 4px" } }, "Big"),
      el("div", { style: { marginBottom: "2px" } }, "if you put \"*\" it will also turn into a bullet point."),
      el("div", { style: { fontFamily: "monospace", background: "#222", padding: "1px 3px", margin: "2px 0", fontSize: "10px" } }, "* text  →  • text")
    )
  );
  overlay.appendChild(modal);
  return overlay;
