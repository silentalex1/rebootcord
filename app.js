  ...
  const sections = [
    { id: "overview", title: "Home Overview", content: () => {
      ...
      if (state.recentKey) {
        ...
        const copyB = el("button", { className: "btn-cancel", style: "font-size:10px; padding:3px 8px", onClick: () => { navigator.clipboard.writeText(state.recentKey); const o = copyB.style.color; copyB.style.color = "var(--green)"; setTimeout(()=>{copyB.style.color=o;},700); } }, "copy key");
      ...
      if (!state.apiKeys || state.apiKeys.length === 0) {
        ...
        state.apiKeys.forEach(k => {
          ...
          const copyB = el("button", { className: "btn-cancel", style: "font-size:10px; padding:3px 8px", onClick: () => { const toCopy = (isRecent && state.lastCreatedKey) ? state.lastCreatedKey.key : (k.masked || "rc_******"); navigator.clipboard.writeText(toCopy); const o = copyB.style.color; copyB.style.color = "var(--green)"; setTimeout(()=>{copyB.style.color=o;},700); } }, "copy key");
          acts.appendChild(showB);
          acts.appendChild(copyB);
...
function renderChangelogModal() {
  ...
  const modal = el("div", { className: "modal changelog-modal" },
    el("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" } },
      ...
      el("button", { className: "btn-cancel", style: { padding: "1px 4px", fontSize: "14px" }, onClick: () => { state.showChangelogModal = false; scheduleRender(); } }, "×")
    ),
