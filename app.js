...
function renderOurApi() {
    ...
    { id: "overview", title: "Home Overview", content: () => {
      ...
      if (!state.apiKeys || state.apiKeys.length === 0) {
        ...
        state.apiKeys.forEach(k => {
          ...
          const copyB = el("button", { className: "btn-cancel", style: "font-size:10px; padding:3px 8px", onClick: () => { navigator.clipboard.writeText(k.masked || "rc_******"); } }, "copy key");
          acts.appendChild(showB);
          acts.appendChild(copyB);
  ...
  setTimeout(() => {
    ...
    if (!existing) {
      ...
      s.onload = () => { if (window.RebootUI) { try { RebootUI.init({apiKey:"demo"}); const root = document.querySelector(".api-container") || document.body; RebootUI.enhance ? RebootUI.enhance(root) : null; const over = document.createElement("style"); over.textContent = ".rb-btn:hover{transform:none!important;filter:none!important} .rb-hover:hover{transform:none!important}"; document.head.appendChild(over); } catch(e){} } };
      document.head.appendChild(s);
    } else if (window.RebootUI) {
      try { RebootUI.init({apiKey:"demo"}); } catch(e){}
    }
  }, 80);
