...
function renderSearchModal() {
  ...
  inp.oninput = function(e) { state.searchTerm = e.target.value; scheduleRender(); };
  ...
  const page = el("div", { className: "projects-page" },
    el("div", { className: "projects-header" },
      ...
      el("div", { style: { display: "flex", gap: "10px", alignItems: "center" } },
        el("input", { className: "search-input", placeholder: "Search project or press ctrl+k", value: state.searchTerm, oninput: function(e){ state.searchTerm = e.target.value; scheduleRender(); }, onfocus: function(){ state.showSearchModal = true; state.searchTerm = ""; scheduleRender(); } }),
...
function renderOurApi() {
  ...
  const navItems = [
    ...
    { id: "hosting-magic", label: "Hosting Magic", icon: "✦" },
    { id: "api-endpoints", label: "API Endpoints", icon: "⟡" },
    { id: "sdks-autostyling", label: "SDK AutoStyling", icon: "◈" },
  ...
  const sections = [
    { id: "overview", label: "Home Overview", content: function() {
      ...
      const keyBtn = el("button", { className: "api-action-btn", onClick: createApiKey },
        `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="7" cy="17" r="3"/><path d="M14 3l7 7-7 7"/><path d="M9.5 13.5l5-5"/></svg> Create API Key`
      );
      keyBtn.querySelector("svg") && (keyBtn.innerHTML = keyBtn.innerHTML);
      d.appendChild(el("div", { style: { marginTop: "16px" } }, keyBtn));

      if (state.recentKey) {
        const box = el("div", { className: "api-key-new-box" });
        box.appendChild(el("div", { className: "api-key-new-label" }, "🔑 Copy this key now — shown only once"));
    ...
    { id: "sdks-autostyling", label: "SDK AutoStyling", content: function() {
      ...
      d.appendChild(codeBlock("1. Include the script", '<script src="https://rebootcord.onrender.com/sdk/rebootui.js"></script>', "html"));
      d.appendChild(codeBlock("2. Initialize", 'RebootUI.init({ apiKey: "YOUR_API_KEY" });', "js"));
      ...
      [
        { icon: "◈", label: "Auto fonts", desc: "Syne + IBM Plex Mono applied site-wide" },
        { icon: "◉", label: "Glass morphism", desc: "Beautiful glass cards and surfaces" },
        { icon: "⟡", label: "Components", desc: "Cards, buttons, inputs, badges, alerts" },
        { icon: "✦", label: "Animations", desc: "Smooth fade-in, slide, pulse effects" },
        { icon: "⧫", label: "Dark mode", desc: "Deep dark theme with accent colors" },
        { icon: "◌", label: "No overrides", desc: "Only affects marked elements, safe to use" }
      ].forEach(function(f) {
        feats.appendChild(el("div", { className: "api-feature-item" },
          el("span", { className: "api-feature-icon" }, f.icon),
          el("div", {},
    ...
    { id: "javascript", label: "Javascript", content: function() {
      ...
      const methods = [
        { name: "RebootUI.init(opts)", desc: "Initialize the SDK. Pass apiKey, accent, font, dark, watch, root." },
