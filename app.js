...
async function sendAIMessage(content) {
  ...
  if (lowerContent.includes('free') || lowerContent.includes('cost') || lowerContent.includes('price')) {
  ...
  } else if (lowerContent.includes('api') || lowerContent.includes('endpoint') || lowerContent.includes('integration') || lowerContent.includes('setup')) {
    setTimeout(() => {
      state.aiChatMessages.push({ role: 'assistant', content: 'To set up the Reboot Cord API integration, you can use our AI Integration tab in the dashboard. We provide a Gemini 2.5 Pro proxy that supports multiple model formats including OpenAI-compatible and Anthropic-compatible endpoints. You can access /v1/chat/completions for chat completions or /v1/messages for Claude-style messages. Check the AI Integration section for full documentation and code examples.' });
      state.aiChatTyping = false;
      scheduleRender();
    }, 500);
  } else if (lowerContent.includes('model') || lowerContent.includes('gpt') || lowerContent.includes('claude') || lowerContent.includes('gemini')) {
    setTimeout(() => {
      state.aiChatMessages.push({ role: 'assistant', content: 'We support a wide range of AI models through our Gemini 2.5 Pro proxy. This includes GPT-4o, GPT-4, Claude Opus, Claude Sonnet, Claude Haiku, Gemini 2.5 Pro, Gemini 2.0 Flash, and many more. The proxy automatically maps these models to the best available Gemini model for your use. Check the AI Integration tab for the full model mapping.' });
      state.aiChatTyping = false;
      scheduleRender();
    }, 500);
  } else if (lowerContent.includes('help') || lowerContent.includes('what can you do') || lowerContent.includes('capabilities')) {
    setTimeout(() => {
      state.aiChatMessages.push({ role: 'assistant', content: 'I can help you with:\n- Setting up API endpoints and integrations\n- Understanding supported AI models and their mappings\n- Troubleshooting common issues\n- Explaining the platform features\n- Providing guidance on Reboot Cord services\n\nJust ask me anything about the Reboot Cord platform and I\'ll do my best to help!' });
      state.aiChatTyping = false;
      scheduleRender();
    }, 500);
  } else {
    try {
      ...
      if (error.message.includes('Gemini proxy not configured')) {
        errorMessage = 'The AI service is currently using a fallback mode. I can still help you with common questions about API setup, models, and platform features. For advanced AI capabilities, the administrator needs to configure the GEMINI_API_KEY environment variable.';
      } else if (error.message.includes('Rate limited')) {
        errorMessage = 'You have reached the rate limit for AI requests. Please wait a moment before trying again.';
      } else if (error.message.includes('payload too large')) {
        errorMessage = 'Your message is too long. Please try with a shorter message.';
      }
