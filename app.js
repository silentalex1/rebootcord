...
function renderChangelogsPage() {
  ...
  if (state.viewChangelogSlug) {
    ...
    if (ch) {
      ...
      wrap.appendChild(el("button", { className: "btn-cancel", onClick: () => { history.pushState(null, "", "/changelog"); state.viewChangelogSlug = null; scheduleRender(); } }, "Go Back"));
      wrap.appendChild(el("h1", { style: { fontSize: "24px" } }, ch.title));
      ...
      wrap.appendChild(el("div", { className: "ch-meta" }, "by " + ch.author + " • " + new Date(ch.ts).toLocaleString()));
  ...
  if (state.isAdmin) {
    header.appendChild(el("div", { style: { display: "flex", gap: "10px", alignItems: "center" } },
      ...
      el("button", { className: "btn-cancel", onClick: () => { history.pushState(null, "", "/dashboard"); state.viewChangelogSlug = null; state.page = "projects"; scheduleRender(); } }, "Go Back")
    ));
  } else {
    header.appendChild(el("div", { style: { display: "flex", gap: "10px", alignItems: "center" } },
      el("button", { className: "btn-cancel", onClick: () => { history.pushState(null, "", "/dashboard"); state.viewChangelogSlug = null; state.page = "projects"; scheduleRender(); } }, "Go Back")
  ...
  if (!state.changelogs || state.changelogs.length === 0) {
    ...
    state.changelogs.forEach(function(ch) {
      ...
      const titleEl = el("div", { className: "ch-title" }, ch.title);
      if (ch.hasLink) {
        const s = ch.slug || ch.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
        titleEl.style.color = "#5865f2";
...
function renderOurApi() {
  ...
  const sidebar = el("div", { className: "api-sidebar", style: "width:220px; flex-shrink:0; border-right:2px solid #333; padding-right:16px; position:relative; padding-top:4px" });
  ...
  const sections = [
    { id: "overview", title: "Home Overview", content: () => {
      ...
      if (state.recentKey) {
        ...
        const keyEl = el("div", { style: "font-family:monospace; word-break:break-all; margin:8px 0; color:#fff; background:#111; padding:8px; border-radius:6px; font-size:12px" }, state.recentKey);
      ...
      if (!state.apiKeys || state.apiKeys.length === 0) {
        ...
        state.apiKeys.forEach(k => {
          ...
          row.appendChild(el("span", { style: "font-family:monospace; color:#888" }, "rc_live_" + "*".repeat(32)));
          const copyM = el("button", { className: "btn-cancel", style: "font-size:10px; padding:4px 8px" , onClick: () => { navigator.clipboard.writeText("rc_live_" + "*".repeat(32)); } }, "Copy masked");
    ...
    { id: "sdks-autostyling", title: "SDKs - AutoStyling", content: () => {
      ...
      d.appendChild(el("div", { style: "margin-bottom:12px; padding:12px; background:#0a0a0a; border-radius:8px; border:1px solid #222" },
        ...
        (() => { const ta = el("textarea", { className: "api-code", style: "width:100%; height:60px; color:#e63946; background:#0f0f0f" }, '<script src="https://rebootcord.onrender.com/sdk/rebootui.js"></script>'); ta.readOnly = true; ta.onclick = () => { navigator.clipboard.writeText(ta.value); const orig = ta.style.color; ta.style.color = "var(--green)"; setTimeout(() => { ta.style.color = orig; }, 800); }; return ta; })()
      ));
      d.appendChild(el("div", { style: "margin-bottom:12px; padding:12px; background:#0a0a0a; border-radius:8px; border:1px solid #222" },
        el("div", { style: "font-weight:600; margin-bottom:6px; color:#fff" }, "Then init:"),
        (() => { const initTa = el("textarea", { className: "api-code", style: "width:100%; height:50px; color:#e63946; background:#0f0f0f" }, 'RebootUI.init({ apiKey: "YOUR_API_KEY" });'); initTa.readOnly = true; initTa.onclick = () => { navigator.clipboard.writeText(initTa.value); const orig = initTa.style.color; initTa.style.color = "var(--green)"; setTimeout(() => { initTa.style.color = orig; }, 800); }; return initTa; })()
      ));
      d.appendChild(el("div", { style: "margin-bottom:12px; padding:12px; background:#0a0a0a; border-radius:8px; border:1px solid #222" },
        el("div", { style: "font-weight:600; margin-bottom:6px; color:#fff" }, "Example:"),
        (() => { const ex = el("div", { className: "api-code", style: "padding:8px; color:#e63946; background:#0f0f0f; white-space:pre" }, '<div class="user-card">Welcome</div>\n\nBecomes:\n<div class="rb-card rb-glass rb-hover">Welcome</div>'); ex.onclick = () => { navigator.clipboard.writeText(ex.textContent); const orig = ex.style.color; ex.style.color = "var(--green)"; setTimeout(() => { ex.style.color = orig; }, 800); }; return ex; })()
      ));
      d.appendChild(el("div", { style: "margin-bottom:12px; padding:12px; background:#0a0a0a; border-radius:8px; border:1px solid #222" },
        el("div", { style: "font-weight:600; margin-bottom:6px; color:#fff" }, "Or:"),
        (() => { const codeEx = el("textarea", { className: "api-code", style: "width:100%; height:40px; color:#e63946; background:#0f0f0f" }, 'RebootUI.createCard({ title: "Bot", description: "Online" });\nRebootUI.page({ theme: "cyber", glass: true });'); codeEx.readOnly = true; codeEx.onclick = () => { navigator.clipboard.writeText(codeEx.value); const orig = codeEx.style.color; codeEx.style.color = "var(--green)"; setTimeout(() => { codeEx.style.color = orig; }, 800); }; return codeEx; })()
      ));
