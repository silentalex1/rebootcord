...
function renderCard(server) {
    ...
    card.innerHTML =
        ...
        '<div class="card-actions">' +
            '<button class="btn-manage" onclick="window.location.href=\'/dashboard/' + encodeURIComponent(server.slug) + '\'">Manage</button>' +
            '<button class="btn-delete-card" onclick="deleteProject(event, \'' + escapeHTML(server.slug) + '\', \'' + escapeHTML(server.name) + '\')">Delete</button>' +
        '</div>';
    return card;
}
...
async function deleteProject(e, slug, name) {
    e.stopPropagation();
