...
function renderLogs(logs) {
    ...
    logs.forEach(entry => {
        ...
        const type = entry.m.startsWith('[ERR]') ? 'err' : entry.m.startsWith('[SYS]') ? 'sys' : entry.m.startsWith('[PKG]') ? 'pkg' : 'out';
...
async function installPkg() {
    ...
    if (data.success) {
        hint.textContent = 'Installing... check console for progress';
        hint.className = 'pkg-hint ok';
        input.value = '';
        startPolling();
    } else {
