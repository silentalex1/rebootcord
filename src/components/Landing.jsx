...
import { HardDrive, Zap, KeyRound, Globe } from "lucide-react";
...
export default function Landing({ onLogin }) {
  ...
  async function handleLogin(e) {
    ...
    if (!url || !key) return toast.error("Panel URL and API Key required");
    setLoading(true);
    try {
      const res = await fetch("/api/servers", {
        headers: { "x-panel-url": url, "x-api-key": key },
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err?.errors?.[0]?.detail || `HTTP ${res.status}`);
      }
      toast.success("Connected to panel");
      onLogin({ panelUrl: url, apiKey: key });
    } catch (err) {
    ...
    <div className="relative min-h-screen overflow-hidden bg-[#1e140a] text-[#f0f0f0]">
      ...
      <nav className="flex items-center justify-between px-8 h-20 border-b-4 border-black bg-[#3b3b3b] relative z-10">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-bold text-[#bcbcbc] uppercase tracking-wider">
          <a href="https://pterodactyl.io" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">Pterodactyl</a>
        </div>
      </nav>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-10">
            ...
            >
              ...
              <span className="text-[10px] uppercase tracking-[0.2em]">Pterodactyl Powered</span>
            </motion.div>
            ...
            <div className="space-y-4">
              ...
              >
                Connect your Pterodactyl panel and manage your Discord bot servers. Real-time logs, file editor, and power controls — all in one place.
              </motion.p>
            </div>
          </div>
          ...
          >
            <div className="bg-black/60 p-8 space-y-6 border-4 border-black">
              <div className="border-b-4 border-black pb-6">
                ...
                <div className="text-[10px] text-[#bcbcbc] font-bold uppercase tracking-widest">Connect your Pterodactyl instance</div>
              </div>

              <form onSubmit={handleLogin} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-[#7a7a7a] flex items-center gap-2">
                    <Globe className="h-3 w-3" /> Panel URL
                  </label>
                  <input
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-[#7a7a7a] flex items-center gap-2">
                    <KeyRound className="h-3 w-3" /> Client API Key
                  </label>
                  <input
                    className="mc-input w-full font-mono text-sm"
                    placeholder="ptlc_..."
      ...
      <footer className="h-24 px-8 flex items-center justify-between border-t-4 border-black bg-[#1a1a1a] text-xs font-bold text-[#7a7a7a] uppercase tracking-widest relative z-10">
        ...
        <div className="flex gap-8">
          <a href="https://pterodactyl-api-docs.netvpx.com/docs/intro" target="_blank" rel="noreferrer" className="text-white hover:text-[#54a332] cursor-pointer">API DOCS</a>
        </div>
      </footer>
    </div>
  );
